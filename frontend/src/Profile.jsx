...
function Profile() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8081/get/' + employeeIndex)
      .then(res => {
        setData({
          ...data,
  ...
  const handleSubmit = (event) => {
    ...
    axios.put('http://localhost:8081/update/' + employeeIndex, data)
      .then(res => {
        if (res.data.Status === "Success") {
          navigate('/Employee'); // Redirect to /Employee after a successful update
        }
      })
      .catch(err => console.log(err));
  };
